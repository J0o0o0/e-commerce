...
namespace e_commerce.Services.Implementation
{
    public class ProductService : IProductService
    {
        ...
        public async Task<PagedResultDto<GetProductDto>> GetFilteredAsync(ProductQueryDto query)
        {
            var result = await _unitOfWork.Products.GetFilteredAsync(query);

            return new PagedResultDto<GetProductDto>
            {
                Data = result.Data.Select(MapToDto).Where(p => p.IsActive == true).ToList(),
                Total = result.Total
            };
        }
        ...
        public async Task<IEnumerable<GetProductDto>> GetBySellerAsync(int userId)
        {
            var seller = await _unitOfWork.Users.GetSellerByUserIdAsync(userId);
            var products = await _unitOfWork.Products.GetBySellerIdAsync(seller.Id);
            return products.Select(MapToDto);
        }
        ...
        public async Task UpdateAsync(int id, UpdateProductDto dto, int userId)
        {
            ...
            var seller = await _unitOfWork.Users
                .GetSellerByUserIdAsync(userId);
